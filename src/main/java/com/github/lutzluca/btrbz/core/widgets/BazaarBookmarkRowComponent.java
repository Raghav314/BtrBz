...
final class BazaarBookmarkRowComponent extends BaseParentUIComponent implements PersistentMouseCapture {
    static final int HEIGHT = 17;
    private static final int ICON_SIZE = 16;
    private static final int DOT_SIZE = 5;

    private final BazaarData.Bookmark bookmark;
    ...
    BazaarBookmarkRowComponent(
        BazaarData.Bookmark bookmark,
        BazaarWidgetOptions.Bookmarks options,
        boolean interactive,
        int index,
        BazaarData.BookmarkDragController drag,
        ...
        this.children = this.showItem ? List.of(this.item) : List.of();
    ...
    public boolean onMouseDown(MouseButtonEvent click, boolean doubled) {
        ...
        if (click.button() == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
            if (this.reorderable) {
                this.drag.start(this.bookmark.productId(), this.index);
                return true;
            }
            this.actions.accept(new BazaarAction.OpenBookmark(this.bookmark.productId()));
            return true;
        }
        if (click.button() == GLFW.GLFW_MOUSE_BUTTON_RIGHT) {
            this.actions.accept(new BazaarAction.RemoveBookmark(this.bookmark.productId()));
            return true;
        }
    ...
    public boolean onMouseDrag(MouseButtonEvent click, double deltaX, double deltaY) {
        if (!this.reorderable || !this.drag.dragging(this.bookmark.productId())) return false;
        this.drag.markMoved();
    ...
    public boolean onMouseUp(MouseButtonEvent click) {
        ...
        if (!result.moved()) {
            this.actions.accept(new BazaarAction.OpenBookmark(result.productId()));
        } else {
            this.actions.accept(new BazaarAction.ReorderBookmark(result.productId(), result.dropIndex()));
        }
        return true;
    }
    ...
    public boolean hasPersistentMouseCapture() {
        return this.reorderable && this.drag.dragging(this.bookmark.productId());
    }
    ...
    public void draw(OwoUIGraphics graphics, int mouseX, int mouseY, float partialTicks, float delta) {
        ...
        if (this.drag.dragging(this.bookmark.productId())) {
            graphics.fill(this.x, this.y, this.x + this.width, this.y + this.height, BazaarStyles.ROW_DRAG);
        }
        if (this.showItem) this.drawChildren(graphics, mouseX, mouseY, partialTicks, delta, this.children);
        ...
        graphics.text(
            font,
            ellipsize(this.productName, available),
            textX,
            this.y + (this.height - font.lineHeight) / 2,
            BazaarStyles.PRIMARY_TEXT,
            false
        );
