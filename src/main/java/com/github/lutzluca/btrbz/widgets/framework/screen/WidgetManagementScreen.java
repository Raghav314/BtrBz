...
import com.github.lutzluca.btrbz.widgets.framework.ui.RestorableVerticalScrollContainer;
import com.github.lutzluca.btrbz.widgets.framework.ui.WidgetChrome;
import com.github.lutzluca.btrbz.widgets.framework.ui.WidgetColorFormat;
import com.github.lutzluca.btrbz.widgets.framework.ui.WidgetSurfaces;
...
public class WidgetManagementScreen extends BaseOwoScreen<FlowLayout> {
    ...
    private void rebuildSidebar() {
        ...
        if (selected != null) {
            ...
            var configurationPanel = selected.configurationPanel();
    ...
    private void addBaseScale() {
        ...
        slider.onChanged().subscribe(this.stateStore::setGlobalFineTuneScale);
        this.sidebarContent.child(slider);
    }

    private void addPlacementProfileControl(WidgetDefinition<?, ?> selected) {
        if (selected.placementProfiles().size() <= 1) return;
        String current = this.placementProfile(selected);
        this.sidebarContent.child(button("Placement: " + placementProfileName(current), control -> {
    ...
    private static String placementProfileName(String profile) {
        return WidgetScreenSession.DEFAULT_PLACEMENT_PROFILE.equals(profile) ? "Container" : "Sign";
    }
    ...
    private void addGameplayEnabledControl(WidgetDefinition<?, ?> selected) {
        ...
        enabled.onChanged().subscribe(value -> this.stateStore.setActive(selected, value));
    ...
    private void addBackgroundEditor(
        ...
        hex.onChanged().subscribe(text -> {
            ...
            this.stateStore.setBackgroundColor(selected, parsed.getAsInt());
            picker.selectedColor(Color.ofArgb(parsed.getAsInt()));
            synchronizing[0] = false;
        });
        picker.onChanged().subscribe(color -> {
            this.stateStore.setBackgroundColor(selected, color.argb());
